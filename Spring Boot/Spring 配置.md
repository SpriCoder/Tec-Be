...
- [1. application.properties和applicaton.yml之间的差别](#1-applicationproperties和applicatonyml之间的差别)
- [2. 如何调整properties转换到yml文件](#2-如何调整properties转换到yml文件)
- [3. 常量配置](#3-常量配置)
  - [3.1. 配置自定义常量](#31-配置自定义常量)
  - [3.2. 大量的常量绑定](#32-大量的常量绑定)
- [4. 配置文件内编程](#4-配置文件内编程)
- [5. 文件外部配置](#5-文件外部配置)
...
# 1. application.properties和applicaton.yml之间的差别
1. 对于application.properties:他是一个表结构。
2. 对于application.yml:天然树状结构，配置和使用更加方便。
3. 同时存在的优先级为:properties > yml

# 2. 如何调整properties转换到yml文件
1. 首先输出application.properties文件后，使用`maven -X clean`删除当前编译文件，保证之后不会出现冲突。
2. clean后创建yml，再次进行汇编。

# 3. 常量配置

## 3.1. 配置自定义常量
1. 我们可以在application.properties中声明自己的全局常量从而在其他地方使用
2. 自定义常量
```
user.name = "user"
user.action = "have a show"
```
3. 使用常量(@Value绑定)
```java
@RestController
...
## 3.2. 大量的常量绑定
1. 绑定一个bean对象(`ConfigBean.java`)
2. 类注解:`@ConfiguraitionProperties(prefix = "user")`:表示前缀
3. 在Spring启动类或者上述bean对象类注解:`@EnableConfigurationProperties({ConfigBean.class})`:指定加载类
4. 之后使用装配这个bean就行
...
# 5. 文件外部配置
1. 修改tomcat端口:`java -jar xx.jar --server.port = 9090`
...
3. 想要使用对应的环境，只需要在`application.properties`中使用`spring.profiles.active`属性来设置，值对应上面提到的`{profile}`，这里就是指dev、prod这2个。 当然你也可以用命令行启动的时候带上参数：`java -jar xxx.jar --spring.profiles.active=dev`
4. 除了可以用profile的配置文件来分区配置我们的环境变量，在代码里，我们还可以直接用@Profile注解来进行配置，例如数据库配置，这里我们先定义一个接口
```java
...
//通过配置文件激活 spring.profiles.active=testdb
@RestController
@RequestMapping("/task")
...
spring.jpa.hibernate.ddl/dl-auto= # DDL mode. This is actually a shortcut for the "hibernate.hbm2ddl.auto" property. Default to "create-drop" when using an embedded database, "none" otherwise.
